const base64Img = require('base64-img'),
  ...
  { COLOR_A, COLOR_B, DEFAULT_STYLE, BASE_URL, STATIC_BASE_URL } = BADGE_OPTIONS;
...
module.exports = {
  /**
   *
   * Generate a dynamic badge for your Holopin profile
  ...
  generateBadge: async (username = DEFAULT_USERNAME, style = DEFAULT_STYLE) => {
    const profileUrl = getProfileUrl(username),
      ...
      badgeURL = `${STATIC_BASE_URL}${sanitizedUsername}-${badges.data.count}-critical?${qs}`;
   ...
   * @returns Holopin README Badge
   */
   generateStaticBadge: async (username = DEFAULT_USERNAME, style = DEFAULT_STYLE) => {
    const profileUrl = getProfileUrl(username),
      sanitizedUsername = username.replace(/[^a-zA-Z0-9 ]/g, ''), // remove special chars otherwise shields will throw an error
      qs = `link=${profileUrl}&logo=${logo}&colorA=${COLOR_A}&colorB=${COLOR_B}&style=${style}`,
